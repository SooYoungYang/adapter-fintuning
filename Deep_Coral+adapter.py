...
import utils
from Deep_Coral_loss import CorrelationAlignmentLoss
...
def add_adapter(model):
    ...
    adapter_module = Adapter(adapter_size=512)
...
def freeze_except_adapter(model):
    for layer, param in model.named_parameters():
        if not layer.startswith("bottleneck") and not layer.startswith("head"):
...
def main(args: argparse.Namespace):
    ...
    backbone = utils.get_model(args.arch) ## backbone
    pool_layer = nn.Identity() if args.no_pool else None
    classifier = utils.ImageClassifier(backbone, num_classes, freeze_bn=args.freeze_bn, dropout_p=args.dropout_p,
                                       finetune=args.finetune, pool_layer=pool_layer).to(device)  ## classifier
    
    if args.adapter: 
        classifier = add_adapter(classifier)
        freeze_except_adapter(classifier)
    
    print(classifier)
    print_trainable_layers(classifier)
...
def train(train_iter: ForeverDataIterator, model, optimizer, lr_scheduler: CosineAnnealingLR,
    ...
    for i in range(args.iters_per_epoch):
        ...
        # compute output
        y_all, f_all = model(x_all) ## y_all은 알겠는데, f_all은 머지?
        ...
        for domain_i in range(n_domains_per_batch):
            ...
            loss_ce += F.cross_entropy(y_i, labels_i)  ## classifier loss 
            # update acc
...
if __name__ == '__main__':
    ...
    parser.add_argument("--adapter", action='store_true', help='adapter training' )
